...
def main():
    ...
    # Navigation avec état pour permettre la redirection programmatique
...
def generate_exercises_page():
    ...
    # Contrôles sous la barre de menu pour accéder au mode réponse
    responder_mode = st.radio(
        "Mode d'affichage",
        ["Génération", "Répondre aux exercices"],
        horizontal=True,
        key="responder_mode_radio"
    )
    if responder_mode == "Répondre aux exercices":
        respond_page()
        st.markdown("---")
        st.stop()
    ...
    if uploaded_file is not None or use_previous_pdf:
        ...
        if texte:
            ...
            with col1:
                # Pré-remplissage depuis une éventuelle régénération automatique
                ...
                # Champ filière pour les étudiants universitaires
            ...
            if st.session_state.get("auto_generate"):
                ...
                if resume:
                    # Stockage et téléchargement comme dans le flux bouton
            ...
            if st.button("🚀 Générer les exercices", type="primary"):
                with st.spinner("Génération en cours..."):
                    # Génération des consignes selon le type
                    if type_exercice == "QCM":
                        consignes = generate_qcm_instructions(nom_matiere, niveau, nb_questions, filiere)
                    elif type_exercice == "QRO":
                        consignes = generate_qro_instructions(nom_matiere, niveau, nb_questions, filiere)
                    ...
                    if resume:
                        # Stocker dans la session pour le mode interactif
                        st.session_state["generated_text"] = resume
                        st.session_state["generated_type"] = type_exercice
                        st.session_state["generated_meta"] = {
                            "matiere": nom_matiere,
                        ...
                        st.session_state["source_pdf_text"] = texte
                        st.session_state["answers"] = {}
                        # Persister une copie pour réutilisation ultérieure (même après navigation)
                        st.session_state["persisted_questions_text"] = resume
                        st.session_state["persisted_questions_type"] = type_exercice
                        # Génération du PDF
                        if type_exercice == "Épreuves":
...
def parse_qcm(text: str) -> List[Dict[str, Any]]:
    ...
    for line in text.splitlines():
        ...
        if not s:
            continue
        # question ligne commençant par chiffre.
        if re.match(r"^\d+\.", s):
            if current:
...
def correct_with_model():
    ...
    consignes = (
        ...
        "Respects les formats et instructions donnés.\n"
    )
    prompt = (
        "EXERCICES GÉNÉRÉS:\n" + generated_text + "\n\n" +
    ...
    if result:
        ...
        # Stocker durablement la correction pour réutilisation ailleurs
        ...
        # (Bouton de régénération supprimé sur demande)
    else:
